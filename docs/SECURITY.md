...
| `pkg/kustomize/kustomize.go`            | `kustomize edit fix --vars` (see [CI.md](CI.md)'s Kustomize Fix section — the one CLI dependency in this repo treated as a hard failure, not a graceful skip, if missing; also invokes `prettier.Write`, i.e. `prettier --write`, as a formatting follow-up) |
