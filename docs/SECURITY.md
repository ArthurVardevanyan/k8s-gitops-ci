...
| `0o644`                                                                  | General file writes (sorted configs, normalized `kustomization.yaml`, extracted schema/policy files, generated Kyverno CLI test fixtures, scaffold README updates)                                       | `pkg/config`, `pkg/kustomize`, `pkg/lint/kubeconform/schemas`, `pkg/lint/kyverno` (×2), `pkg/scaffold`                  |
