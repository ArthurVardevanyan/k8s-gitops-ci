...
Examples: `pkg/lint/kyverno`'s
`ExcludedRules`/`IncludeComponents`/`NamespaceSelectorLabelKeys`,
`pkg/validator/static/psa`'s `ValidModes`/`ValidLevels`,
...
Worked example: `pkg/validator/static/namespace/resource_scope.go` (generated
by `scripts/pull-scoped-resources.sh` from `kubectl api-resources`
against a live cluster; core, generic Kinds only) plus
`pkg/validator/static/namespace/extra_resource_scope.go` (an empty, exported
...
`clusterScopeNamespaceExempt` in `pkg/validator/static/namespace/namespace.go`)
...
| `image:build`             | Build a single-arch (native) image locally (quick iteration, validates Containerfile changes before a release; production image building uses GitHub Actions)                                                                                                                             |
| `image:publish`           | Build multi-arch image + push manifest (linux/amd64 + linux/arm64) via `podman manifest`; the same pattern as [docker-ntp/build-multiarch-podman.sh](https://github.com/ArthurVardevanyan/docker-ntp/blob/main/build-multiarch-podman.sh) (production image building uses GitHub Actions) |
