...
| `image:build`             | Build a single-arch (native) image locally (quick iteration, validates Containerfile changes before a release)                                                                                                                            |
| `image:publish`           | Build multi-arch image + push manifest (linux/amd64 + linux/arm64) via `podman manifest`; the same pattern as [docker-ntp/build-multiarch-podman.sh](https://github.com/ArthurVardevanyan/docker-ntp/blob/main/build-multiarch-podman.sh) |
