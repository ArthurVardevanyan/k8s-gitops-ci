...
| ------------------ | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `image-checksum`   | `pkg/validator/static/image`                                                                                           | Yes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `cluster-name`     | `pkg/validator/static/clusterid` (a cluster-identity sub-finding)                                                      | Yes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `project-ref`      | `pkg/validator/static/clusterid` (a project-identity sub-finding)                                                      | Yes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `cel`              | The CEL validation step (`pkg/validator/phases.go`) — validates `x-kubernetes-validations` from embedded CRD schemas . | **No — deliberately non-exemptable.** CEL rules are enforced by the Kubernetes API server at admission time, so an exemption in CI would not prevent the cluster from rejecting the manifest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `cluster-identity` | `pkg/validator/static/clusterid` (an overlay's own identity field not matching its cluster folder)                     | Yes — a value substituted at sync time by a GitOps controller (e.g. a placeholder such as `INFRA_ID` checked into every overlay of an app) legitimately won't match the overlay's folder name, so `check=cluster-identity,value=INFRA_ID` in a `test.sh` `EXEMPTIONS=(...)` block (or an `exempt-cluster-identity` annotation naming the exact value) is the intended escape hatch.                                                                                                                                                                                                                                                                                                                 |
| `image-fqdn`       | `pkg/validator/static/image`                                                                                           | **No — deliberately non-exemptable.** See [Annotation exemption](#1-annotation-exemption) above for the rationale.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `invalid-json`     | `pkg/validator/static/clusterid` (any `*.json` file under the overlay that fails to parse)                             | **No — deliberately non-exemptable.** `exempt.Exemptable` hardcodes this ID to always return `false`, and `RegisterExemptable` refuses to register it even if called. This is intentional: a syntactically-broken JSON file is a hard error, not something a selector or annotation should be able to wave away.                                                                                                                                                                                                                                                                                                                                                                                    |
...
older hardcoded special case used by `invalid-json` (see the table
above).
...
3. **Only use a dedicated `exempt.ID*` constant instead of the check's
   ...
   `clusterid`'s `cluster-name`/`project-ref`/`cluster-identity`/`invalid-json`
   split) —
