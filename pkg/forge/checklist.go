...
var checkedBoxLinePattern = regexp.MustCompile(`(?i)^\s*-\s*\[x\]\s*(.*)`)
...
func buildCheckedSet(body string, items []ChecklistItem) map[string]bool {
	...
	for _, line := range strings.Split(body, "\n") {
		m := checkedBoxLinePattern.FindStringSubmatch(line)
		if m == nil {
			continue
		}
		rest := strings.ToLower(m[1])
		for _, item := range items {
			target := strings.ToLower(strings.TrimLeft(item.LabelPattern, " \t"))
