...
// TestResolveChangeset_TargetModeTakesPriorityOverFullScan guards that
// Apps/Clusters (build-yaml --app/--cluster) still takes priority over
// FullScan, matching the existing Apps/Clusters-over-Dirs priority - full
// scan mode must never override an explicit targeted build.
func TestResolveChangeset_TargetModeTakesPriorityOverFullScan(t *testing.T) {
	d := t.TempDir()
	app := filepath.Join(d, "myapp")
	mustWrite(t, filepath.Join(app, "base", "deployment.yaml"), "kind: Deployment\n")

	files, err := resolveChangeset(Options{Apps: []string{app}, FullScan: true})
	if err != nil {
		t.Fatalf("resolveChangeset: %v", err)
	}
	if len(files) != 1 || filepath.Base(files[0]) != "deployment.yaml" {
		t.Fatalf("expected only the targeted app's file (FullScan ignored), got %d: %v", len(files), files)
	}
}
