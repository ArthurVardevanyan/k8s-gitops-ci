...
// runKyvernoValidation writes every successfully-rendered overlay's YAML
// (collected during this phase's overlay build loop - see buildOverlayWithHooks's
// rendered return value) to a temp directory and runs the kyverno CLI
// against the whole batch in one invocation. This is always a non-blocking
// advisory (see kyverno.FormatComment's own doc comment) - a missing CLI,
// missing/unpreparable policies, or any per-file write failure all degrade
...
func runKyvernoValidation(outputs []kyvernoOutput, log *logger.Logger) Section {
	...
	// a reviewer can act on rather than an ephemeral temp path.
